...
/* Make selectbox taller and unit visible */
.stSelectbox > div {
...
# ═══════════════════════════════════════════════════════════════
CP_TO_LBM_FT_H = 2.4191

DEFAULTS = {
    "m_h": 60000.0, "m_c": 89143.0,
    "T_hin": 250.0, "T_hout": 150.0, "T_cin": 85.0, "T_cout": 120.0,
    "Do_in": 1.0, "Di_in": 0.834, "L_ft": 20.0, "PT_in": 1.25,
    "B_ratio": 0.30, "n_p": 4,
    "Rd_shell": 0.002, "Rd_tube": 0.002,
    "UD_est": 40.0, "DP_limit": 10.0,
    "Cp_shell": 0.52, "k_shell": 0.074, "mu_shell_cp": 2.75, "rho_shell": 49.0,
    "Cp_tube": 1.00, "k_tube": 0.37, "mu_tube_cp": 0.70, "rho_tube": 62.4,
}
...
# Unit mapping for each input key
UNIT_MAP = {
    "m_h": "mass_flow", "m_c": "mass_flow",
    "T_hin": "temperature", "T_hout": "temperature", "T_cin": "temperature", "T_cout": "temperature",
    "Do_in": "length", "Di_in": "length", "L_ft": "length", "PT_in": "length",
    "rho_shell": "density", "rho_tube": "density",
    "Cp_shell": "cp", "Cp_tube": "cp",
    "k_shell": "k", "k_tube": "k",
    "mu_shell_cp": "viscosity", "mu_tube_cp": "viscosity",
...
def conv_vis(v, u): return v*1000 if u == "mPa·s" else v
def conv_visb(v, u): return v/1000 if u == "mPa·s" else v
...
class FP:
    def __init__(self, name, cp, k, mu_cp, rho):
        self.name = name
        self.Cp = clamp(cp)
        self.k = clamp(k)
        self.mu_cp = clamp(mu_cp)
        self.mu = self.mu_cp * CP_TO_LBM_FT_H
        self.rho = clamp(rho)
        self.s = self.rho / 62.4
...
def kern(inp, sh, tu):
    ...
    A_req = Q / clamp(UD * dT)
    nt = max(int(round(A_req / (math.pi*Do*L) / 2) * 2), 20)
    CTP = 0.93
    Ds = max(round(math.sqrt(clamp(nt*(PT**2)/(0.785*CTP)))*12/2)*2, 8)
    Ds_ft = Ds / 12
    A_act = nt * math.pi * Do * L
    U_req = Q / clamp(A_act * dT)

    Re_t = max(abs(4*(m_c/clamp(nt/np_))/(math.pi*Di*tu.mu)), 1)
    if Re_t > 10000: hi = (tu.k/Di)*0.023*sp(Re_t,0.8)*sp(tu.Pr,1/3)
    elif Re_t > 2100: hi = (tu.k/Di)*0.116*(sp(Re_t,2/3)-125)*sp(tu.Pr,1/3)*(1+sp(Di/L,2/3))
    else: hi = (tu.k/Di)*1.86*sp(Re_t*tu.Pr*Di/L,1/3)
    hi = max(float(np.real(hi)), 5)

    B = clamp(Br * Ds_ft, 0.05)
    Ccl = PT - Do
    a_s = clamp(Ds_ft * Ccl * B / PT)
    G_s = abs(m_h / a_s)
    de = clamp((4*(PT**2 - math.pi*Do**2/4))/(math.pi*Do))
    Re_s = max(abs(de * G_s / sh.mu), 1)

    jH = max(0.5*(1+Br)*(0.08*sp(Re_s,0.6821)+0.7*sp(Re_s,0.1772)), 1)
    ho = max(float(np.real(jH*(sh.k/de)*sp(sh.Pr,1/3))), 5)

    kw = 26.0
    Uc = 1/(Do/clamp(hi*Di) + Do*math.log(Do/Di)/(2*kw) + 1/ho)
    Rd = Rdt*(Do/Di) + Rds
    UDf = 1/(1/Uc + Rd)
    ...
    G_t = m_c / clamp(nt*math.pi*Di**2/4/np_)
    ft = 0.4137*sp(Re_t,-0.2585) if Re_t>3000 else 16/Re_t
    DP_ft = (ft*np_*L*G_t**2)/(7.5e10*Di*clamp(tu.s))
    DP_rt = 1.334e-13*(2*np_-1.5)*G_t**2
    DP_tube = DP_ft + DP_rt

    nb = max(int(round(L/B))-1, 1)
    f1 = (0.0076+0.000166*Ds)*sp(Re_s,-0.125)
    f2 = (0.0016+5.8e-5*Ds)*sp(Re_s,-0.157)
    fs = max(float(np.real(f1-1.25*(1-Br)*(f1-f2))), 1e-9)
    DP_shell = (fs*G_s**2*Ds_ft*(nb+1))/(7.5e10*de*clamp(sh.s))
    ...
    return {
        ...
        "B":float(B*12), "nb":int(nb), "s_s":float(sh.s), "s_t":float(tu.s),
    }
...
def delaware(inp, sh, nt, Ds):
    ...
    PT = clamp(inp["PT_in"])
    Bc, rss = 0.20, 0.10
    B = max(float(inp["B_ratio"])*Ds, 1)
    L_ft = clamp(inp["L_ft"])
    nb = max(int(round(L_ft/clamp(B/12)))-1, 1)
...
def sens(base, sh, tu):
    ...
    x = np.linspace(0.15, 0.60, 32)
    o["baf"] = (x, [safe({**base,"B_ratio":v},sh,tu)["DP_s"] for v in x])
    x = np.linspace(0.5, 20.0, 32)
    o["vis"] = (x, [safe(base,FP(sh.name,sh.Cp,sh.k,v,sh.rho),tu)["DP_s"] for v in x])
    return o

def make_chart(base, sh, tu):
    s = sens(base, sh, tu)

    # Get units from session state
    um = st.session_state.get("u_m_h", "lb/h")
    ul = st.session_state.get("u_Do_in", "in")
    udp = st.session_state.get("u_DP_limit", "psi")

    # Professional chart with axis labels
    fig = make_subplots(
        rows=2, cols=3,
        subplot_titles=(
            "<b>Mass Flow Rate</b>",
    ...
    # Mass Flow - Row 1 Col 1
    x, ds, dt = s["flow"]
    fig.add_trace(go.Scatter(
        x=x, y=ds, name="Shell ΔP", mode="lines",
        line=dict(color=purple, width=3),
        fill='tozeroy', fillcolor='rgba(124,58,237,0.1)'
    ), row=1, col=1)
    fig.add_trace(go.Scatter(
        x=x, y=dt, name="Tube ΔP", mode="lines",
    ...
    # Tube OD - Row 1 Col 2
    x, ds, dt = s["do"]
    fig.add_trace(go.Scatter(x=x, y=ds, showlegend=False, mode="lines",
        line=dict(color=purple, width=3),
        fill='tozeroy', fillcolor='rgba(124,58,237,0.1)'
    ), row=1, col=2)
    fig.add_trace(go.Scatter(x=x, y=dt, showlegend=False, mode="lines",
        line=dict(color=cyan, width=3, dash="dash"),
        fill='tozeroy', fillcolor='rgba(6,182,212,0.05)'
    ...
    # Shell Viscosity - Row 2 Col 2
    x, ds = s["vis"]
    fig.add_trace(go.Scatter(x=x, y=ds, showlegend=False, mode="lines",
        line=dict(color=purple, width=3),
        fill='tozeroy', fillcolor='rgba(124,58,237,0.1)'
    ), row=2, col=2)
    fig.update_xaxes(title_text="Viscosity (cP)", row=2, col=2)
...
<div class="nav-container">
    <div class="nav-content">
        ...
        <div class="nav-links">
            <a class="nav-link-a active" href="#home">Home</a>
            <a class="nav-link-a" href="#input">Input</a>
            <a class="nav-link-a" href="#kern">Kern Method</a>
            <a class="nav-link-a" href="#delaware">Delaware</a>
            <a class="nav-link-a" href="#sensitivity">Sensitivity</a>
        </div>
    </div>
</div>
""", unsafe_allow_html=True)
...
<div class="hero-card">
    <div class="hero-content">
        ...
        <div class="hero-tags">
            ...
            <span class="hero-tag">Group 2 - Section 01</span>
        </div>
    </div>
</div>
""", unsafe_allow_html=True)
...
<div class="section">
    <div class="section-header">
        ...
        <div>
            ...
            <p class="section-subtitle">Enter thermal and physical parameters</p>
        </div>
    </div>
</div>
""", unsafe_allow_html=True)
...
def inp_row(label, key, group, min_v=None, step=None, fmt="%.3f"):
    ...
    # Get short unit display name
    unit_display = u.replace("Btu/lbm·°F","Btu/lb·°F").replace("h·ft²·°F/Btu","h·ft²·°F/Btu")

    st.markdown(f'<div class="input-label-text">{label}</div>', unsafe_allow_html=True)
    col1, col2 = st.columns([2.5, 1])
    with col1:
        st.number_input("__", value=float(dv), min_value=min_v, step=step, format=fmt, key=f"in_{key}", label_visibility="collapsed")
...
with col2:
    ...
    st.markdown('<div class="input-label-text">Tube Length (ft)</div>', unsafe_allow_html=True)
...
c3, c4 = st.columns(2)
with c3:
    st.markdown('<div class="input-label-text">Baffle Ratio (B/Ds)</div>', unsafe_allow_html=True)
    st.slider("__", 0.15, 0.60, st.session_state["v_B_ratio"], 0.01, key="v_B_ratio", label_visibility="collapsed")
...
with c4:
    ...
    inp_row("Viscosity Shell", "mu_shell_cp", "viscosity", 0.0001, 0.01, "%.4f")
    inp_row("Viscosity Tube", "mu_tube_cp", "viscosity", 0.0001, 0.01, "%.4f")
    inp_row("Density Shell", "rho_shell", "density", 0.1, 1.0, "%.3f")
    inp_row("Density Tube", "rho_tube", "density", 0.1, 1.0, "%.3f")
...
sh = FP("Shell", st.session_state["v_Cp_shell"], st.session_state["v_k_shell"], st.session_state["v_mu_shell_cp"], st.session_state["v_rho_shell"])
tu = FP("Tube", st.session_state["v_Cp_tube"], st.session_state["v_k_tube"], st.session_state["v_mu_tube_cp"], st.session_state["v_rho_tube"])
...
<div class="section">
    <div class="section-header">
        ...
        <div>
            ...
            <p class="section-subtitle">Shell-side calculations using Kern approach</p>
        </div>
    </div>
</div>
""", unsafe_allow_html=True)
...
kern_data = [
    ("Q", kr["Q"]/1e6, "MMBtu/h", "Heat Duty", "🔥"),
    ("LMTD", kr["LMTD"], ut, "Log-mean Temp", "🌡️"),
    ("F Factor", kr["F"], "-", "Correction", "📐"),
...
<div class="section">
    <div class="section-header">
        ...
        <div>
            ...
            <p class="section-subtitle">Advanced shell-side calculations with correction factors</p>
        </div>
    </div>
</div>
""", unsafe_allow_html=True)
...
<div class="section">
    <div class="section-header">
        ...
        <div>
            ...
            <p class="section-subtitle">Pressure drop variation with design parameters</p>
        </div>
    </div>
</div>
""", unsafe_allow_html=True)
...
<div class="footer">
    ...
    <div>CHPE4412 • Group 2 • Section 01 • Spring 2026</div>
    <div>Naeema Al Amri (141056) • Jumana Al Maani (142225)</div>
</div>
""", unsafe_allow_html=True)
