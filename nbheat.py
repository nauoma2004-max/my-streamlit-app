...
class FP:
    def __init__(self, name, cp, k, mu_lb_ft_h, sg):
        ...
        self.Cp = clamp(cp)
        self.k = clamp(k)
        self.mu = clamp(mu_lb_ft_h)  # Given directly in lb/ft-h (no conversion)
        self.sg = clamp(sg)
        self.rho = self.sg * 62.4  # Convert SG to lb/ft³
        self.s = self.sg  # Specific gravity
...
def kern(inp, sh, tu):
    ...
    Do = clamp(inp["Do_in"]) / 12  # ft
    Di = clamp(inp["Di_in"]) / 12  # ft
    L = clamp(inp["L_ft"])
    PT = clamp(inp["PT_in"]) / 12  # ft
    np_ = max(int(round(inp["n_p"])), 1)
    Rdt = max(float(inp["Rd_tube"]), 0)
    Rds = max(float(inp["Rd_shell"]), 0)

    # Fixed parameters from Example 5.6
    nt = int(inp.get("n_tubes", 310))
    Ds = float(inp.get("Ds_in", 23.25))  # in
    kw = float(inp.get("k_metal", 26.0))
    B_in = float(inp.get("B_space_in", 6.975))  # in
    ...
    # Tube Mass Velocity (lb/h-ft2) - Reference PHT 2.py
    G_t = (m_c * np_/nt) / (math.pi * Di**2 / 4)
    Re_t = max(abs(Di * G_t / tu.mu), 1)
    # Dittus-Boelter for turbulent flow
    hi = (tu.k/Di)*0.023*sp(Re_t,0.8)*sp(tu.Pr,1/3)
    hi = max(float(np.real(hi)), 5)

    B = B_in / 12  # ft
    # de = 0.0792 ft given directly in Example 5.6
    de = 0.0792  # ft
    # Cross-flow area (ft2) - from PHT 2.py reference
    # as = (Ds * (pitch - Do) * B) / 144 where Ds, pitch, Do are in inches
    as_area = (Ds * (PT*12 - Do*12) * B_in) / 144
    G_s = abs(m_h / as_area)
    Re_s = max(abs(de * G_s / sh.mu), 1)

    # Kern jH factor - Reference PHT 2.py
    jh = 0.5 * (1 + B/Ds_ft) * (0.08*sp(Re_s,0.6821) + 0.7*sp(Re_s,0.1772))
    ho = max(float(np.real(jh*(sh.k/de)*sp(sh.Pr,1/3))), 5)

    # Overall coefficient - Reference PHT 2.py
    ...
    # Tube Pressure Drop - Reference PHT 2.py
    ft = 0.4137*sp(Re_t,-0.2585)
    dp_f = (ft * np_ * L * G_t**2) / (7.50e12 * Di * tu.s)
    dp_r = 1.334e-13 * (2 * np_ - 1.5) * G_t**2 / tu.s
    DP_tube = dp_f + dp_r

    # Shell Pressure Drop - Reference PHT 2.py
    nb_plus_1 = (L * 12 / B_in)  # (nb + 1)
...
def delaware(inp, sh, nt, Ds):
    ...
    PT = clamp(inp["PT_in"])
    Bc, rss = 0.20, 0.10
    B = max(float(inp.get("B_space_in", 6.975)), 1.0)  # Baffle spacing in inches
    L_ft = clamp(inp["L_ft"])
...
def sens(base, sh, tu):
    ...
    x = np.linspace(3.0, 15.0, 32)  # Baffle space in inches
...
def inp_row(label, key, group, min_v=None, step=None, fmt="%.3f"):
    ...
    elif group == "sg": dv = val; ni = lambda x: x  # SG has no conversion
    elif group == "count": dv = int(val); ni = lambda x: int(x)  # Count is integer
    else: dv = val; ni = lambda x: x

    # Ensure min_v, step are floats for number_input compatibility
...
with c3:
    ...
    inp_row("Viscosity Shell (lb/ft-h)", "mu_shell", "viscosity", 0.1, 0.1, "%.3f")
    inp_row("SG Shell", "sg_shell", "sg", 0.1, 0.01, "%.3f")
with c4:
    inp_row("Viscosity Tube (lb/ft-h)", "mu_tube", "viscosity", 0.1, 0.1, "%.3f")
